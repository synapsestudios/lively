...
- **type**: A `string` containing one of: `string`, `integer`, `boolean`, `resumable-upload`, `hash`, `array[hash]`, `array[string]`, `array[boolean]`, `array[integer]`, `stripe_token`, or `enum`. If `enum`, specify `enumValues` property on param containing an array of strings to display as options to the user.
